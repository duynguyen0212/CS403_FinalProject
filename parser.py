...
Modification was made in the parser
-	Line 132, 232-234: I add the token DOT represent the ‘.’ For the lexer to recognize
-	Line 364: Robot AST node
-	Line 837: Checking ROVER type
-	Line 1221-1282: parser for the rover_method
-	Line 1682-1712: rover_robot class for the interpreter
-	Line 2183: visit rover node
...
KEYWORDS = ['VAR', 'AND', 'OR', 'NOT', 'IF', 'ELIF', 'ELSE', 'FOR', 'TO', 'STEP', 'WHILE', 'FUN', 'THEN', 'END',
            'RETURN', 'CONTINUE', 'BREAK', 'ROVER', 'info', 'drill', 'change_map', 'turn_right',
            'turn_left', 'move_forward', 'random_location']
...
class Parser:
    ...
    def call(self):
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
                                                          "Expected ')', 'VAR', 'IF', 'FOR', 'WHILE', 'FUN', int, "
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, 'ROVER'):
            rover_method = res.register(self.rover_method())
            if res.error:
                return res
            return res.success(rover_method)
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'THEN'):
            return res.failure(
                InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected 'THEN'"))
    ...
    def for_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            if not self.current_tok.matches(TT_KEYWORD, 'END'):
                return res.failure(
                    InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected 'END'"))
    ...
    def func_def(self):
        ...
        if self.current_tok.type != TT_NEWLINE:
            return res.failure(
                InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected '->' or NEWLINE"))
    ...
    # Parser for ROVER robot

    def rover_method(self):
        res = ParseResult()
        import main
        if not self.current_tok.matches(TT_KEYWORD, 'ROVER'):
            return res.failure(
                InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected 'ROVER'"))
        res.register_advancement()
        self.advance()
        ...
        if self.current_tok.type == TT_DOT:
            ...
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
                                                  f"Expected 'a dot after ROVER to call its method'"))
        # Check what rover is called in cmd, I tried many ways but
        # it didn't know which rover is using so it only use the
        # default rover1
        call_rover_method = None

        if main.ROVER_COMMAND_FILES[main.rover_name] == "Rover2":
            call_rover_method = rover.rover2
        else:
            call_rover_method = rover.rover1

        if self.current_tok.matches(TT_KEYWORD, 'info'):
            var_name_tok = self.current_tok
            call_rover_method.info()

        elif self.current_tok.matches(TT_KEYWORD, 'random_location'):
            var_name_tok = self.current_tok
            call_rover_method.random_location()

        elif self.current_tok.matches(TT_KEYWORD, 'drill'):
            var_name_tok = self.current_tok
            call_rover_method.drill()

        elif self.current_tok.matches(TT_KEYWORD, 'change_map'):
            var_name_tok = self.current_tok
            res.register_advancement()
            self.advance()
            if self.current_tok.type == TT_IDENTIFIER:
                map_name = self.current_tok
                call_rover_method.change_map(map_name)
            else:
                print("Missing file name")

        elif self.current_tok.matches(TT_KEYWORD, 'turn_right'):
            var_name_tok = self.current_tok
            call_rover_method.turn_right()
        elif self.current_tok.matches(TT_KEYWORD, 'turn_left'):
            var_name_tok = self.current_tok
            call_rover_method.turn_left()
        elif self.current_tok.matches(TT_KEYWORD, 'move_forward'):
            var_name_tok = self.current_tok
            call_rover_method.move_forward()
        else:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
                                                  f"Expected 'a rover's method'"))
        res.register_advancement()
        self.advance()
        if res.error: return res

        return res.success(RoverNode(var_name_tok))
...
class BaseFunction(Value):
    ...
    def check_args(self, arg_names, args):
        ...
        if len(args) > len(arg_names):
            return res.failure(
                RTError(self.pos_start, self.pos_end, f"{len(args) - len(arg_names)} too many args passed into {self}",
                        self.context))

        if len(args) < len(arg_names):
            return res.failure(
                RTError(self.pos_start, self.pos_end, f"{len(arg_names) - len(args)} too few args passed into {self}",
                        self.context))
...
class Rover_robot:
    def __init__(self, rover_method):
        self.context = None
        self.pos_end = None
        self.pos_start = None
        self.set_pos()
        self.set_context()
        self.rover_method = rover_method
    ...
    def execute(self, args):
        res = RTResult()
        interpreter = Interpreter()
        new_context = Context(self.rover_method, self.context, self.pos_start)
        new_context.symbol_table = SymbolTable(new_context.parent.symbol_table)

        rover_method = self.rover_method
        rover_method.set_context(new_context)
        new_context.symbol_table.set(rover_method, None)

        value = res.register(interpreter.visit(rover_method, new_context))
        if res.error: return res
...
class Interpreter:
    ...
    def visit_RoverNode(self, node, context):
        res = RTResult()

        func_name = node.var_name_tok.value
        func_value = Rover_robot(func_name)
